...
- **`ai-tools/`**: AI-assisted development tools. `default.nix` is the index; `codewhale.nix` builds the upstream CodeWhale release binary (version and hash tracked by `nvfetcher`) and installs the hush skill; `conoha-vps-mcp.nix` wires the MCP server with SOPS-injected credentials (`conoha-schema-fix.js`). Enabled with the desktop full stack (`my.home.desktop.full.enable`).
