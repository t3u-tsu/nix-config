...
- **`desktop/`**: Desktop experience — Niri, PipeWire, NetworkManager, GUI apps; lightweight core via `my.services.desktop.enable` / `my.home.desktop.enable` plus an opt-in full stack (`my.services.desktop.full.enable` / `my.home.desktop.full.enable`) for gaming/Unity/creative/media; wires `home/desktop` into Home Manager for the primary user.
