...
- **Full Stack (opt-in)**: `my.services.desktop.full.enable` and `my.home.desktop.full.enable` (per host) opt into the heavier stack — gaming (plus the Chaotic-Nyx overlay), game-engine toolchains in containers (rootless podman), creative, media, office, and AI/hardware dev tools. Intended for machines that can afford the extra disk, build and eval time.
